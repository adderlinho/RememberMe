...
public class RememberMeOneTimeActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private EditText mFriendView;
    private TextInputLayout mFriendLayout;
    //private EditText mNotificationView;
    private Button   mCreateButton;
    private SessionManager manager;
    private ArrayList<ReminderPojo> reminderPojos = new ArrayList<>();
    private PendingIntent pendingIntent;
    private int day_reminder, month_reminder, year_reminder, hour_reminder, min_reminder;
    private Bundle bundle;
    private View mProgressView;
    //private String email_notification = "", phone_notification = "";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_remember_me_one_time);
        ...
        mFriendView       = (EditText) findViewById(R.id.friend);
        mFriendLayout     = (TextInputLayout) findViewById(R.id.friendLayout);
        //mNotificationView = (EditText) findViewById(R.id.notification);
        mCreateButton     = (Button) findViewById(R.id.reminder_create_button);

        bundle = getIntent().getExtras();
        if(bundle != null)
        {
            if(bundle.getString("token_friend") != null)
            {
                mFriendLayout.setVisibility(View.VISIBLE);
                mFriendView.setText(bundle.getString("username_friend"));
            }
            else
            {
...
//                else if(TextUtils.isEmpty(mNotificationView.getText()))
//                {
//                    mNotificationView.setError(getString(R.string.error_invalid_time));
//                    mNotificationView.requestFocus();
//                }
                else
                {
                    ...
                    if(bundle != null)
                    {
                        if(bundle.getString("token_friend") != null)
                        {
                            sendDataReminderFriend(
                                    bundle.getString("token_friend"),
                                    mTitleView.getText().toString(),
                                    mMessageView.getText().toString(),
                                    mDateView.getText().toString(),
                                    mHourView.getText().toString(),
                                    "one_time",
                                    manager.getPreferences(this, "user").toString(),
                                    manager.getPreferences(this, "token_session").toString());
                        }
                        else
                        {
                            if(bundle.getString("id") != null && bundle.getString("alarm_id") != null)
                            {
                            ...
                            {
                                setDataReminderOT(Settings.Secure.getString(RememberMeOneTimeActivity.this.getContentResolver(), Settings.Secure.ANDROID_ID),
...
//        dialog.setContentView(R.layout.model_to_dialog); //Your custom layout
//        dialog.setTitle(getResources().getString(R.string.prompt_notification));
//
//        final CheckBox cbEmail  = (CheckBox) dialog.findViewById(R.id.cbEmail);
//        final CheckBox cbPhone  = (CheckBox) dialog.findViewById(R.id.cbPhone);
//        final Spinner emailSpinner = (Spinner) dialog.findViewById(R.id.emailSpinner);
//        final Spinner phoneSpinner = (Spinner) dialog.findViewById(R.id.phoneSpinner);
//        final Button notification_set_button = (Button) dialog.findViewById(R.id.notification_set_button);
//
//
//        notification_set_button.setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View v) {
//                if(cbEmail.isChecked() && cbPhone.isChecked())
//                {
//                    mNotificationView.setText("Send alert to: email (" + emailSpinner.getSelectedItem().toString() +"), phone (" + phoneSpinner.getSelectedItem().toString() +") before.");
//                    email_notification = emailSpinner.getSelectedItem().toString().replace(" min", "");
//                    phone_notification = phoneSpinner.getSelectedItem().toString().replace(" min", "");
//                }
//                else if(cbEmail.isChecked())
//                {
//                    mNotificationView.setText("Send alert to: email (" + emailSpinner.getSelectedItem().toString() +") before.");
//                    email_notification = emailSpinner.getSelectedItem().toString().replace(" min", "");
//                }
//                else if(cbPhone.isChecked())
//                {
//                    mNotificationView.setText("Send alert to: phone (" + phoneSpinner.getSelectedItem().toString() +") before.");
//                    phone_notification = phoneSpinner.getSelectedItem().toString().replace(" min", "");
//                }
//                else
//                {
//                    mNotificationView.setText("Without notifications.");
//                }
//                dialog.dismiss();
//            }
//        });
...
//    }
    ...
    public void setDataReminderOT(final String user_phone_id, final String user_email, final String title, final String message, final String date, final String time) {
        ...
        Call<ReminderResponse> reminderOTResponseCall = endpointsApi.setDataReminder(user_phone_id, user_email, title, message, date, time, "one_time");
        reminderOTResponseCall.enqueue(new Callback<ReminderResponse>() {
            ...
            public void onResponse(Call<ReminderResponse> call, Response<ReminderResponse> response) {
                ...
                if(reminderResponse != null)
                {
                    ...
                    Intent intentAlarm = new Intent(RememberMeOneTimeActivity.this, AlarmReceiverActivity.class);
                    ...
                    pendingIntent = PendingIntent.getActivity(RememberMeOneTimeActivity.this, 0, intentAlarm, PendingIntent.FLAG_CANCEL_CURRENT);
                    ...
                    calendar.set(Calendar.YEAR, year_reminder);
                    ...
                    Toast.makeText(RememberMeOneTimeActivity.this, getResources().getString(R.string.successfull_server_2), Toast.LENGTH_SHORT).show();
                    Intent intentMain = new Intent(RememberMeOneTimeActivity.this, MainActivity.class);
                ...
                {
                    Toast.makeText(RememberMeOneTimeActivity.this, getResources().getString(R.string.error_server_1), Toast.LENGTH_SHORT).show();
    ...
    public void sendDataReminderFriend(final String token_friend, final String titulo, final String mensaje, final String fecha, final String hora, final String tipo, final String usuario, final String token_session) {
        RestApiAdapter restApiAdapter = new RestApiAdapter();
        Gson gsonMediaRecent = restApiAdapter.construyeGsonDeserializadorRespuesta();
        EndpointsApi endpointsApi = restApiAdapter.establecerConexionRestApiHeroku(gsonMediaRecent);
        Call<RespuestaResponse> respuestaResponseCall = endpointsApi.sendDataReminderFriend(token_friend, titulo, mensaje, fecha, hora, tipo, usuario, token_session);
